...
struct FlowGFWOmegaXi {
  ...
  void process(aodCollisions::iterator const& collision, aod::BCsWithTimestamps const&, aodTracks const& tracks, aod::CascDataExt const& Cascades, aod::V0Datas const& V0s, DaughterTracks&)
  {
    ...
    for (auto& v0 : V0s) {
      ...
      if (v0.qtarm() / TMath::Abs(v0.alpha()) > cfgv0_ArmPodocut &&
          (!cfgcheckDauTPC || (TMath::Abs(v0posdau.tpcNSigmaPi()) < cfgtpcNSigmaCascPion && TMath::Abs(v0negdau.tpcNSigmaPi()) < cfgtpcNSigmaCascPion)) &&
          (!cfgcheckDauTOF || (TMath::Abs(v0posdau.tofNSigmaPi()) < cfgtofNSigmaCascPion && TMath::Abs(v0negdau.tofNSigmaPi()) < cfgtofNSigmaCascPion))) {
      ...
      } else if ((!cfgcheckDauTPC || (TMath::Abs(v0posdau.tpcNSigmaPr()) < cfgtpcNSigmaCascProton && TMath::Abs(v0negdau.tpcNSigmaPi()) < cfgtpcNSigmaCascPion)) &&
                 (!cfgcheckDauTOF || (TMath::Abs(v0posdau.tofNSigmaPr()) < cfgtofNSigmaCascProton && TMath::Abs(v0negdau.tofNSigmaPi()) < cfgtofNSigmaCascPion))) {
      ...
      if (v0.dcaV0daughters() > cfgv0_dcav0dau)
        continue;
      if (TMath::Abs(v0.dcapostopv()) < cfgv0_dcadautopv)
        continue;
    ...
    for (auto& casc : Cascades) {
      ...
      } else if (casc.sign() < 0 && TMath::Abs(casc.yXi()) < cfgCasc_rapidity && TMath::Abs(bachelor.tpcNSigmaPi()) < cfgtpcNSigmaCascPion && TMath::Abs(posdau.tpcNSigmaPr()) < cfgtpcNSigmaCascProton && TMath::Abs(negdau.tpcNSigmaPi()) < cfgtpcNSigmaCascPion && TMath::Abs(bachelor.tofNSigmaPi()) < cfgtofNSigmaCascPion && TMath::Abs(posdau.tofNSigmaPr()) < cfgtofNSigmaCascProton && TMath::Abs(negdau.tofNSigmaPi()) < cfgtofNSigmaCascPion) {
